...
export async function POST(request: NextRequest) {
  ...
  type Row = { id: number; rotation: number; horaire: string; terrain: string }
  const rows = await sql`SELECT id, rotation, horaire, terrain FROM matches WHERE id IN (${matchIdA}, ${matchIdB})` as Row[]
  if (rows.length !== 2) return Response.json({ ok: false }, { status: 400 })
  const a = rows.find(r => r.id === matchIdA)!
  const b = rows.find(r => r.id === matchIdB)!
